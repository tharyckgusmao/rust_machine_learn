...
use tch::vision::imagenet::normalize;
...
impl Iterator for DataLoader {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        for i in start..end {
            ...
            let random_flip = random::<bool>();
            let random_crop = random::<i64>() % 6;

            let random_cutout = random::<i64>() % 6;
            if self.train {
                let image = augmentation(
                    &image_t.unsqueeze(0),
                    random_flip,
                    random_crop,
                    random_cutout
                );
                images.push(image.squeeze());
            } else {
                images.push(image_t);
            }
